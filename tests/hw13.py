...
class Test_hw_13:
    ...
    def test_check_user_agent(self, user_agent, platform, browser, device):
        ...
        assert platform == actual_platform, f"Response parameter 'platform' is NOT correct: {actual_platform}. " \
                                            f"Expected platform: {platform}"
        assert browser == actual_browser, f"Response parameter 'browser' is NOT correct: {actual_browser}. " \
                                          f"Expected browser: {browser}"
        assert device == actual_device, f"Response parameter 'device' is NOT correct: {actual_device}. " \
                                        f"Expected device: {device}"
