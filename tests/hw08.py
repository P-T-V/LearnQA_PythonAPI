...
print(f"Server answer, for 1st request (starting job):\n"
      f"a) Job time = {response1.json()['seconds']}\n"
      f"b) Job token = {response1.json()['token']}\n")
data = {
    'token': job_token
}
response2 = requests.get(
    "https://playground.learnqa.ru/api/longtime_job", params=data)
...
print(f"Time to sleep: {job_time} seconds")
time.sleep(job_time)
...
response4 = requests.get(
    "https://playground.learnqa.ru/api/longtime_job", params={'token': incorrect_token}    )
